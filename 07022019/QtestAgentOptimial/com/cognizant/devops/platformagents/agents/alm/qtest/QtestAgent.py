...
class QtestAgent (BaseAgent):
            ...
            for project in projectList:
                ...
                for entity in self.almEntities:
                    ...
                    while nextResponse:
                        ...
                        entityTypeList = response.get('items', response.get('defects', []))
                        print entityTypeList
                        if len(entityTypeList) > 0:
                            injectData = {'projectId': projectId, 'projectName': projectName, 'almType': entity}
                            self.responseDataWrangling(entityConfig, entityTypeList, toolsData, injectData, entityUpdatedDate, projectTrackingDetails)
                            page = page + 1
                        else:
                            nextResponse = False
                    if toolsData:
    ...
    def responseDataWrangling(self, entityConfig, entityTypeList, toolsData, injectData, entityUpdatedDate, trackingDetails):
        ...
        for entityObject in entityTypeList:
            ...
            if entityConfig:
                ...
                if entityObject.get('properties', []):
    ...
    def queryEarliestchangeHistoryStrategy(self, entityObjectIdList, **injectData):
        try:
            toolsHistoryData = list()
            projectId = injectData.get('projectId')
            entity = injectData.get('almType')
            page = 1
            propertyLookUp = self.historyEntities.get(entity, {}).get("Type")
            objectQueryChunks = self.constructHistoryObjectQuery(entityObjectIdList)
            for chunk in objectQueryChunks:
                if chunk:
                    nextResponse = True
                    while nextResponse:
                        historyResponse = self.queryObjectHistoryApi(projectId, entity, objectQuery=chunk, page=page, pageSize=self.pageSize)
                            ...
                            for changeHistory in changeHistoryList:
                                ...
                                for changedField in changesList:
                                    if changedField['new_value'] == propertyLookUp['newValue']:
                                        data = {"projectId": projectId, "almType": entity, "id": changeHistory['linked_object']['object_id'], "automationTime": changeHistory['created']}
    ...
    def constructHistoryObjectQuery(self, entityIdList):
        if len(entityIdList) > self.entitySize:
            ...
            chunks = [entityIdList[_Iter: _Iter + self.entitySize] for _Iter in range(0, len(entityIdList), self.entitySize)]
            for _Iter in chunks:
