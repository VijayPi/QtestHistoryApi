...
class QtestAgent(BaseAgent):
                ...
                for projects in projectsList:
                    ...
                    if len(almEntities) > 0:
                        ...
                        for entityType in almEntities:
                            ...
                            # Changed
                            reqIdDict = trackingDetails.get(entityType, {}).get("entityIdDict", dict())
                            reqIdHistoryList = trackingDetails.get(entityType, {}).get("entityIdHistoryDict", list())
                            # Changed end
                                            ...
                                            if lastUpdated is not None:
                                                ...
                                                if lastUpdated > startFrom:
                                                    ...
                                                    if responseTemplate:
                                                        ...
                                                        # changed
                                                        # if str(res.get('id')) not in reqIdDict:
                                                        ...
                                                        data += self.parseResponse(responseTemplate, res,
                                                                                   injectData)
                                                        objectId = str(res.get('id'))
                                                        if objectId not in reqIdDict:
                                                            reqIdDict[objectId] = res.get('last_modified_date')
                                                        elif reqIdDict.get(objectId, None) < \
                                                                res.get('last_modified_date', None):
                                                            print objectId, '*' * 12
                                                            reqIdHistoryList.append(objectId)
                                                        # changed end
                            ...
                            if entity_type_available and entityUpdatedDate is not None:
                                # changed
                                if len(reqIdDict) > 0 and entityType in extensions.get('linkedArtifacts', {}).get(
                                        'almEntities', {}):
                                    trackingDetails[entityType] = {"entityUpdatedDate": entityUpdatedDate,
                                                                   "entityIdDict": reqIdDict,
                                                                   "entityIdHistoryDict": reqIdHistoryList}
                                else:
                                    trackingDetails[entityType] = {"entityUpdatedDate": entityUpdatedDate}
                                # changed end
                            if len(data) > 0:
                            ...
                            if reqIdHistoryList:
                                print reqIdHistoryList
                                self.typePropertyhistoryApi(projectId, entityType, reqIdHistoryList)
                        self.tracking[str(projectId)] = trackingDetails
    ...
    def typePropertyhistoryApi(self, projectId, objectType, objectIdList):
        ...
        changeHistory = json.loads(self.queryObjectHistoryApi(projectId, objectType, objectQuery=obectQuery).content)
        print changeHistory
